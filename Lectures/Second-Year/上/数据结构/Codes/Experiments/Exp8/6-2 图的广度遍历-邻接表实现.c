...
/* 你的代码将被嵌在这里 */
void BFS(ALGraph *G, int v) {
	int Q[MAX_VERTEX_NUM], front = 0, rear = 0; //定义顺序队列
	//访问v结点并修改访问状态
	printf(" %d", v);
	visited[v] = TRUE;
	Q[rear++] = v; //v入队并修改队尾指针
	while (front != rear) { //队列不为空
		int temp = Q[front++];//获取队首元素
		ArcNode *p = G->vertices[temp].firstarc; //求temp的第一个邻接点
		//访问该层——类似于二叉树的层序遍历
		while (p != NULL) {
			if (visited[p->adjvex] == FALSE) { //如果该结点未访问过
				//访问邻接点并修改访问状态
				printf(" %d", p->adjvex);
				visited[p->adjvex] = TRUE;
				Q[rear++] = p->adjvex; //将访问的结点存到队列中
			}
